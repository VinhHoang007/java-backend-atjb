...
<div class="row">
	<div class="col-12 ">
		<p>
			<a href="">Home</a>>> <a style="font-weight: bold; color: gray;">Edit
				Post</a>
		</p>
	</div>
</div>
			...
			<div class="col-12">
				...
				<form:form action="editPost" method="post"
					...
					<div>
						...
						<div>
							<textarea name="content" rows="8" cols="80"><c:out
									value="${postEdited.content}" /></textarea>
						</div>
					</div>
					...
					<div>
						<label for="tags">Tags </label><br> <input type="text"
							id="tags" name="tags" value="${postEdited.tags}">
					</div>
					...
					<div style="margin-bottom: 5px;">
						...
						<select name="status">
							<option value="${postEdited.status}" selected hidden><c:choose>
									<c:when test="${postEdited.status == '1'}">draft</c:when>
									<c:when test="${postEdited.status == '2'}">published</c:when>
